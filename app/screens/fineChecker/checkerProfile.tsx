...
const CheckerProfile = () => {
  ...
  const loadCheckerData = async () => {
    try {
      ...
      if (checker) {
        setName(checker.fullName);
        setEmail(checker.email || "");
        setCheckerId(checker.checkerId || "");
        setMobileNumber(checker.mobileNumber || "");
        setMunicipalCouncil(checker.municipalCouncil || "Colombo");
        setProfilePictureUrl(checker.profilePictureUrl || null);
      }
    } catch (error) {
      console.error("Error loading checker data:", error);
      Alert.alert("Error", "Failed to load checker details");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCancel = () => {
    ...
    setName((params.checkerName as string) || "");
      ...
      <SafeAreaView style={styles.safeArea} edges={["bottom"]}>
        ...
        >
          ...
          >
            ...
            {/* Title */}
            <Text style={styles.title}>Fine Checker Profile</Text>
            ...
            <View style={styles.formContainer}>
              ...
              {/* Checker ID Input (Non-editable) */}
              <View style={styles.inputGroup}>
                <Text style={styles.label}>Checker ID</Text>
                <View style={styles.lockedInputContainer}>
                  <TextInput
                    style={[styles.input, styles.inputLocked]}
                    placeholder="CHK10779"
                    placeholderTextColor="#999"
                    value={checkerId}
                    editable={false}
                  />
